...
if($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['aggiungi'])){
    $stmt = $conn->prepare("INSERT INTO destinazioni (citta, paese, prezzo, data_partenza, data_ritorno) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param(
        "sssss",
        $_POST['citta'],
        $_POST['paese'],
    ...
    $stmt->execute();
    echo "<div class='alert alert-success'>Destinazione aggiunta correttamente!</div>";
}
...
if($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['salva_modifica'])){
    $stmt = $conn->prepare("UPDATE destinazioni SET citta=?, paese=?, prezzo=?, data_partenza=?, data_ritorno=? WHERE id=?");
    $stmt->bind_param(
        "sssssi",
        $_POST['citta'],
        $_POST['paese'],
            ...
            <div class="row g-3">
                ...
                <div class="col-12">
                    <button name="<?= $destinazione_modifica ? 'salva_modifica' : 'aggiungi' ?>" 
                            ...
                            type="submit">
                        <?= $destinazione_modifica ? 'Salva Modifica' : 'Aggiungi Destinazione' ?>
                    </button>
                </div>
            </div>
        </form>
        ...
        <?php while($row = $result->fetch_assoc()): ?>
            <tr>
                ...
                <td>
                    ...
                    <a class="btn btn-sm btn-danger" href="?elimina=<?= $row['id'] ?>" onclick="return confirm('Sicuro di voler eliminare questa destinazione?')">Elimina</a>
                </td>
            </tr>
        <?php endwhile; ?>
    </tbody>
